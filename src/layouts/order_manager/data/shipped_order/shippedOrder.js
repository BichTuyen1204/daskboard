...
export default function shippedOrder(pageOnShipped, rowsPerPageOnShipped) {
  ...
  useEffect(() => {
    ...
    fetchOrders();
  }, [jwtToken, pageOnShipped, rowsPerPageOnShipped]);

  const hasNextPageOnShipped = pageOnShipped < totalPages;
  ...
  const rows = orders.map((item) => ({
    ...
    action: (
      <MDBox display="flex" justifyContent="center">
        <Link to={`/order_shipped/${item.id}`} style={{ textDecoration: "none" }}>
          <MDTypography
            variant="caption"
            style={{
  ...
  return { columns, rows, hasNextPageOnShipped };
}
