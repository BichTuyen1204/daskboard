...
export default function confirmOrder(pageOnConfirm, rowsPerPageOnConfirm) {
  ...
  useEffect(() => {
    ...
    fetchOrders();
  }, [jwtToken, pageOnConfirm, rowsPerPageOnConfirm]);

  const hasNextPageOnConfirm = pageOnConfirm < totalPages;
  ...
  const rows = orders.map((item) => ({
    ...
    status: (
      <MDTypography variant="caption" style={{ color: "green" }}>
        <strong>{item.order_status === "ON_CONFIRM" ? "ON CONFIRM" : item.order_status}</strong>
      </MDTypography>
    ),
    action: (
  ...
  return { columns, rows, hasNextPageOnConfirm };
}
